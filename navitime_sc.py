...
for ken_code in range(1,48): #都道府県の繰り返し処理。エンド値は「未満」を示すことに注意！
    print(ken_code)
    hajime = 'https://www.navitime.co.jp/category//0504001/{}'.format(str(ken_code).zfill(2)) # ★
    url = hajime
    ken = str(ken_code).zfill(2)
    if ken_code in [1,12,13,14,23,27,28,]:  # 750件以上結果がある県はここで除外設定しないといけない
        print("pass")
        pass
    else:
...
ken = [11, 13, 14, 23, 27]
for ken_code in ken:
    for t in range(101,564): #　◆ 最大値＋１を入力すること！
